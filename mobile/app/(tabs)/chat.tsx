...
export default function ChatScreen() {
  ...
  const handleSend = async (text = input) => {
    ...
    try {
      ...
      const aiMessage = { id: Date.now() + 1, text: response.message, sender: 'ai' };
  ...
  const waveStyle = useAnimatedStyle(() => {
    const scale = interpolateMetering(metering);
    return {
      transform: [{ scale }],
  ...
  const interpolateMetering = (m) => {
    'worklet';
    // Mapping metering (-160 to 0) to scale (1 to 2.5)
    const normalized = Math.max(0, (m + 160) / 160);
        ...
        <View className="space-y-4 pb-10">
          ...
          {loading && (
            <View className="flex-row justify-start">
              <View className="bg-white border border-slate-100 p-4 rounded-2xl rounded-tl-none">
